...
#               curl -sS --request PUT --url https://api.'$URL_TEXT'datacite.org/dois/'$PREFIX'%2F'$SUFFIX" --user '"$USER_PASS"' --header 'content-type: application/vnd.api+json' -d @"$JSON_FILE_NAME
       ...
       printf "\n%s\t%s\t%s\t%s\t%s\t%s\n\n"  $DOI $PREFIX $SUFFIX_LOWER $PRODUCT_KEY_UPPER $JSON_FILE_NAME $SPASE_RESOURCE_ID

       datacite_json_doi_info_all.com "$DOI_LOWER" $SPASE_RESOURCE_ID

       spase_editor_update.com /Users/kunchan/DOI_001 TEST $SPASE_RESOURCE_ID.xml
