...
if [ -e $REGISTRY_DIRECTORY/$SPASE_RESOURCE_ID.xml ]; then
   ...
   /Users/kunchan/bin/spase_xml_tab_wrapper.com $REGISTRY_DIRECTORY/WORK/spase_editor_update_old_$$.xml

   RELEASE_DATE_PRIOR=`grep ^'\ \ \ \ \ \ \ \ \ <ReleaseDate>' $REGISTRY_DIRECTORY/WORK/spase_editor_update_old_$$.xml | awk 'BEGIN { FS="[<>]" } ; { print $3 }' | sed 's/Z$//'`

   /Users/kunchan/bin/spase_xml_tab_wrapper.com $REGISTRY_DIRECTORY/WORK/spase_editor_update_new_$$.xml
...
if [ $SPASE_RESOURCE_TYPE != "Person" ]; then
   ...
   echo '<Spase xmlns="http://www.spase-group.org/data/schema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.spase-group.org/data/schema http://www.spase-group.org/data/schema/spase-2_5_0.xsd">' >> $COLLATION_FILE_NAME

   echo '   <Version>2.5.0</Version>' >> $COLLATION_FILE_NAME
   ...
   if [ $DOI_NUM -eq 0 ]; then
      ...
      AUTHORS=`grep '	'$SPASE_RESOURCE_ID'	' DATACITE/datacite_json_doi_info_all.tab | awk 'BEGIN { FS="	" } ; { print $7 }'`

      DOI=`grep '	'$SPASE_RESOURCE_ID'	' DATACITE/datacite_json_doi_info_all.tab | awk 'BEGIN { FS="	" } ; { print $2 }'`

      PUBLICATION_DATE=`grep '	'$SPASE_RESOURCE_ID'	' DATACITE/datacite_json_doi_info_all.tab | awk 'BEGIN { FS="	" } ; { print $4 }'`

      if [ ! $PUBLICATION_DATE ]; then PUBLICATION_DATE=2022; fi

      PUBLISHED_BY=`grep '	'$SPASE_RESOURCE_ID'	' DATACITE/datacite_json_doi_info_all.tab | awk 'BEGIN { FS="	" } ; { print $5 }'`
...
do
    ...
    CURL_CHECK=`/Users/kunchan/bin/curl_check.com $URL`
...
/Users/kunchan/bin/doi_check.com $COLLATION_FILE_NAME | sed 's/^/SPASE_EDITOR_/'
